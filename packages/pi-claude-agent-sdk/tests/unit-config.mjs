...
describe("loadConfig", () => {
	...
	it("merges folder profiles by name", () =>
		withTempHome(() => {
			const cwd = mkdtempSync(join(tmpdir(), "claude-bridge-project-"));
			try {
				const globalDir = getAgentDir();
				...
				writeFileSync(
					...
					JSON.stringify({
						profiles: {
							...
							work: { claudeDir: "/profiles/old-work" },
						},
					}),
				);
				writeFileSync(
				...
				assert.deepEqual(loadConfig(cwd).profiles, {
					personal: { claudeDir: "~/.claude-personal" },
					work: { claudeDir: "/profiles/work" },
				});
			} finally {
	...
	it("markStartupNoticeShown records today's date without dropping existing settings", () =>
		withTempHome(() => {
			const cwd = mkdtempSync(join(tmpdir(), "claude-bridge-project-"));
			try {
				const globalDir = getAgentDir();
	...
	it("markStartupNoticeShown leaves an unparseable config untouched", () =>
		withTempHome(() => {
			const globalDir = getAgentDir();
			mkdirSync(globalDir, { recursive: true });
			const path = join(globalDir, "claude-bridge.json");
	...
	it("markStartupNoticeShown creates the config when there is none", () =>
		withTempHome(() => {
			const cwd = mkdtempSync(join(tmpdir(), "claude-bridge-project-"));
			try {
