/* Generated by scripts/build-plugins.mjs from plugins/vscode-editor/src/index.ts - do not edit. */

// plugins/vscode-editor/src/index.ts
...
var MAX_LIST_ENTRIES = 8e3;
var MAX_DEPTH = 12;
var MAX_READ_BYTES = 2 * 1024 * 1024;
var MAX_SSH_HOSTS = 32;
var CONN_TIMEOUT_MS = 15e3;
var MAX_EXEC_OUTPUT = 256 * 1024;
var MAX_DOWNLOAD_BYTES = 100 * 1024 * 1024;
var MAX_UPLOAD_BYTES = 100 * 1024 * 1024;
var UPLOAD_STALE_MS = 30 * 60 * 1e3;
function toWire(p) {
...
var index_default = {
  activate(host) {
    ...
    async function renameEntry(rel, newName) {
      if (typeof newName !== "string" || !newName.trim() || newName.includes("/") || newName.includes("\\") || newName.includes("..")) {
    ...
    const sftpCfgFile = () => path.join(sftpCfgDir(), "sftp.json");
    const LEGACY_SYNC_STORE = path.join(host.dir, "sync-configs.json");
    const syncConns = /* @__PURE__ */ new Map();
    let syncConnFp = "";
    ...
    function normalizeCfg(c) {
      ...
      return {
        ...
        exclude: Array.isArray(c.ignore ?? c.exclude) ? [...new Set((c.ignore ?? c.exclude).map(String))].filter(Boolean) : []
      };
    }
      ...
      return new Promise((res) => {
        ...
        void (async () => {
          try {
            ...
            const child = cli ? spawn(process.execPath, [cli, ...args], { stdio: "ignore" }) : spawn(process.platform === "win32" ? "npm.cmd" : "npm", args, { stdio: "ignore" });
            ...
            async function finish(ok) {
              ...
              host.notify(
                ...
                syncDeps.ok ? "📝 Editor sync dependency installation completed" : "📝 Editor sync dependency installation failed; run npm install ssh2 in the plugin directory"
              );
    ...
    function makeIgnoreMatcher(patterns) {
      const rules = (patterns ?? []).map(String).filter(Boolean).map((raw) => {
        ...
        if (!pat.includes("/")) {
          list.push(globToRegExp(`**/${pat}`));
          list.push(globToRegExp(`${pat}/**`));
          list.push(globToRegExp(`**/${pat}/**`));
        }
        if (pat.endsWith("/**")) list.push(globToRegExp(pat.slice(0, -3)));
        return list;
      });
    ...
    async function mkdirpRemote(sftp, rpath) {
      ...
      for (const s of segs) {
        cur = cur === "." ? s : `${cur}/${s}`;
        await sftpCall(sftp, "mkdir", cur).catch(() => {
        });
      }
    }
    async function runSyncTransfer(cfg, direction, scope, targetRel, onProgress) {
      ...
      if (scope === "file") {
        ...
        rels = direction === "up" ? await collectLocal(baseRel, cfg) : await collectRemote(sftp, posixJoin(cfg.remoteRoot || "/", baseRel), baseRel, cfg);
      }
      const failed = [];
      let done = 0;
    ...
    let sshCfgs = null;
    const sshConns = /* @__PURE__ */ new Map();
    let nextSshConn = 1;
      ...
      if (sec?.get) {
        for (const h of sshCfgs.hosts) {
          ...
          for (const [field] of SECRET_FIELDS) {
            if (!h[field]) {
              const name = hostSecretName(h.id, field);
              const v = name ? sec.get(name) : void 0;
              if (v !== void 0) h[field] = v;
            }
          }
        }
      }
    ...
    async function saveSshCfgs() {
      const hosts = sec ? (sshCfgs?.hosts ?? []).map((h) => {
        const clean = { ...h };
    ...
    async function beginUpload(clientId, msg) {
      ...
      if (msg.connId) {
        ...
        let exists2 = false;
        try {
          exists2 = (await sftpCall(await getSftp(c), "stat", rpath)).isFile();
        } catch {
        }
        uploads.set(key, {
          key,
        ...
        return { uploadId, exists: exists2 };
      }
    ...
    async function chunkUpload(clientId, msg) {
      ...
      if (u.scope === "local") {
        ...
        u.fh = null;
        await fs.rename(u.tmp, u.finalAbs);
      } else {
        const sftp = await getSftp(getSshConn(u.connId));
        await mkdirpRemote(sftp, u.rpath.split("/").slice(0, -1).join("/"));
        ...
        switch (action) {
          ...
          case "download": {
            if (!msg.connId) {
              ...
              if (st2.size > MAX_DOWNLOAD_BYTES)
                throw new Error(`file exceeds ${Math.round(MAX_DOWNLOAD_BYTES / 1024 / 1024)}MB limit`);
              const buf = await fs.readFile(abs);
              host.sendTo(clientId, { res: true, reqId, ok: true, action, b64: buf.toString("base64"), size: st2.size });
              break;
            }
            const c = getSshConn(msg.connId);
          ...
          case "read": {
            const r = msg.connId ? await remoteRead(getSshConn(msg.connId), String(msg.path ?? "")) : await readFile(msg.path);
          ...
          case "sync_save": {
            ...
            if (!String(c.remoteRoot ?? "").trim().startsWith("/"))
            ...
            const next = normalizeCfg({
              ...
              name: c.name !== void 0 ? String(c.name || "") : old.name ?? "",
              ...
              privateKeyPath: c.privateKeyPath !== void 0 ? String(c.privateKeyPath || "").trim() : old.privateKeyPath ?? "",
              agent: c.agent !== void 0 ? String(c.agent || "") : old.agent ?? "",
          ...
          case "sync_run": {
            ...
            const summary = await runSyncTransfer(
              cfg,
              direction,
              scope,
              msg.path ?? "",
          ...
          case "hosts_save": {
            ...
            if (h.id) {
              ...
              storeHostSecret(h.id, "password", h.password === null ? null : h.password || void 0);
              storeHostSecret(h.id, "privateKey", h.privateKey === null ? null : h.privateKey || void 0);
              sshCfgs.hosts[i] = {
                ...old,
                ...
                // details = details null = details
                password: h.password === null ? void 0 : h.password || old.password,
                privateKey: h.privateKey === null ? void 0 : h.privateKey || old.privateKey
              };
            } else {
              ...
              storeHostSecret(id, "password", h.password || void 0);
              storeHostSecret(id, "privateKey", h.privateKey || void 0);
              sshCfgs.hosts.push({
                id,
                ...
                password: h.password ? String(h.password) : void 0,
                privateKey: h.privateKey ? String(h.privateKey) : void 0
              });
            }
            await saveSshCfgs();
            broadcastSshState();
          ...
          case "connect": {
            ...
            void connectSshHost(cfg, clientId, reqId);
            return;
          }
          case "disconnect":
          ...
          case "shell_resize":
            try {
              getSshConn(msg.connId).streams.get(msg.shellId)?.setWindow(Number(msg.rows) || 24, Number(msg.cols) || 80, 0, 0);
            } catch {
            }
            return;
          case "exec":
