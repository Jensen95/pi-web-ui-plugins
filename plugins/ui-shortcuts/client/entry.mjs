...
var STORAGE_KEY = "pi-web-ui.ui-shortcuts.bindings";
var MODIFIERS = {
  ctrl: "Ctrl",
  control: "Ctrl",
  cmd: "Meta",
...
var MODIFIER_ORDER = ["Ctrl", "Meta", "Mod", "Alt", "Shift"];
var VIEW_NAMES = /^(?:chat|terminal|git|plugin:[a-z0-9][a-z0-9._-]*)$/i;
...
function normalizeShortcut(raw) {
  ...
  for (const part of parts) {
    ...
    if (modifier) {
      if (modifiers.includes(modifier)) return void 0;
  ...
  if (!key || modifiers.length === 0) return void 0;
  if (modifiers.includes("Mod") && (modifiers.includes("Ctrl") || modifiers.includes("Meta"))) return void 0;
...
function eventKey(event) {
  if (typeof event.key !== "string") return void 0;
  return canonicalKey(event.key) ?? (event.key.length === 1 ? event.key.toUpperCase() : void 0);
}
...
function bindingForEvent(event, bindings = DEFAULT_BINDINGS) {
  if (!event || event.defaultPrevented || event.repeat || event.isComposing) return void 0;
...
function shortcutForEvent(event) {
  ...
  if (!action || action.type !== "view") return void 0;
  return SHORTCUTS.find((shortcut) => shortcut.view === action.view);
}
function validView(view) {
  return VIEW_NAMES.test(view.trim());
}
function customId(shortcut, label) {
  const slug = `${shortcut}-${label}`.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
  return `custom-${slug || "shortcut"}`;
}
function createBinding(shortcut, label, actionType, value) {
  const normalized = normalizeShortcut(shortcut);
...
function normalizeStoredBinding(raw) {
  ...
  if (!id || !shortcut || !label || !sourceAction || typeof sourceAction !== "object") return void 0;
...
function readCustomBindings(storage) {
  if (!storage) return [];
  try {
    ...
    const seenShortcuts = /* @__PURE__ */ new Set();
...
function writeCustomBindings(storage, bindings) {
...
function mergedBindings(custom) {
...
function performAction(host, action) {
  try {
    if (action.type === "view") {
    ...
    return typeof host?.startChat === "function" && host.startChat({ prompt: action.prompt, newChat: action.newChat }) !== false;
  } catch {
    return false;
  }
}
...
function makeInput(document, field, type, placeholder) {
