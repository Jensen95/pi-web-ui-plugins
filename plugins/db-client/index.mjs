/* Generated by scripts/build-plugins.mjs from plugins/db-client/src/index.ts - do not edit. */

// plugins/db-client/src/index.ts
import { dirname, join } from "node:path";
import { spawn } from "node:child_process";
import { createRequire } from "node:module";
import { existsSync, readFileSync, rmSync, writeFileSync } from "node:fs";
import { readFile as rf, writeFile as wf } from "node:fs/promises";
var CONFIG_FILE = "db-connections.json";
var DEPS = ["mysql2@^3", "pg@^8", "mssql@^12", "mongodb@^7", "ioredis@^6"];
var MAX_CONNS = 32;
var MAX_RUNTIME = 8;
var OP_TIMEOUT_MS = 3e4;
var CONNECT_TIMEOUT_MS = 15e3;
var MAX_PAGE_ROWS = 500;
var MAX_QUERY_ROWS = 1e3;
var MAX_CELL_LEN = 4e3;
var INSTALL_LOCK = ".deps-install.lock";
var INSTALL_TIMEOUT_MS = 20 * 6e4;
var INSTALL_LOCK_STALE_MS = 30 * 6e4;
var DB_TYPES = {
...
function withTimeout(promise, ms, label) {
  return Promise.race([
    promise,
    new Promise(
      (_resolve, reject) => setTimeout(() => reject(new Error(`${label ?? "operation"} timed out (${ms / 1e3}s)`)), ms)
    )
  ]);
}
function qMysql(value) {
...
function qMssql(value) {
  return "[" + String(value).replace(/\]/g, "]]") + "]";
}
function qSqlite(value) {
  return '"' + String(value).replace(/"/g, '""') + '"';
}
...
function cellVal(value) {
  ...
  if (typeof value === "object") {
    let s2;
    try {
      s2 = JSON.stringify(value, (_key, item) => {
    ...
    if (s2.length > MAX_CELL_LEN) s2 = s2.slice(0, MAX_CELL_LEN) + "…";
    return s2;
  }
...
function rowsToGrid(columns, rows) {
  return {
    columns,
    rows: rows.map(
      (row) => Array.isArray(row) ? row.map(cellVal) : columns.map((column) => cellVal(row?.[column]))
    )
  };
}
...
function queryMysql(conn, sql, params) {
  return params === void 0 ? conn.query(sql) : conn.query(sql, params);
}
async function mysqlAdapter(cfg) {
  ...
  return {
    ...
    async listDatabases() {
      ...
      return rows.map((r) => Object.values(r)[0]).filter((name) => Boolean(name));
    },
    async listTables(db) {
    ...
    async selectPage(db, t, opt) {
      ...
      const fields = rows.length ? Object.keys(rows[0]) : (await conn.query("SELECT * FROM ??.?? LIMIT 1", [db, t]))[0]?.fields?.map((f) => f.name) ?? (await queryMysql(
        conn,
...
function queryPg(client, sql, params) {
  return params === void 0 ? client.query(sql) : client.query(sql, params);
}
async function postgresAdapter(cfg) {
  const mod = await loadDriver("pg");
  ...
  async function getCli(name) {
    ...
    await withTimeout(c.connect(), CONNECT_TIMEOUT_MS + 3e3, "connect");
    clients.set(name, c);
    return c;
  }
  ...
  return {
    ...
    async describeTable(db, t) {
      ...
      const ddlLines = cols.rows.map(
        (c2) => `  ${qPg(c2.column_name)} ${c2.data_type}${c2.max_len ? `(${c2.max_len})` : ""}${c2.is_nullable === "NO" ? " NOT NULL" : ""}${c2.column_default ? ` DEFAULT ${c2.column_default}` : ""}`
      );
...
async function sqliteAdapter(cfg) {
  ...
  const mod = await loadDriver("node:sqlite");
  ...
  return {
    ...
    async updateRow(_db, t, pkCol, pkVal, changes) {
      ...
      const info = db.prepare(
        `UPDATE ${qSqlite(t)} SET ${cols.map(qSqlite).map((c) => `${c}=?`).join(", ")} WHERE ${qSqlite(pkCol)}=?`
...
async function mssqlAdapter(cfg) {
  const ms = await loadDriver("mssql");
  ...
  async function getPool(name) {
    ...
    await withTimeout(p.connect(), CONNECT_TIMEOUT_MS + 5e3, "connect");
    pools.set(name, p);
    return p;
  }
    ...
    async describeTable(db, t) {
      ...
      return {
        columns: cols.recordset.map((c) => {
          ...
          return {
            name: String(c.name),
            type: String(c.type) + (c.max_len && maxLen > 0 && maxLen < 8e3 ? `(${String(c.max_len)})` : ""),
            nullable: c.nullable === "YES",
...
` + cols.recordset.map((c) => `  ${c.name} ${c.type} ${c.nullable === "YES" ? "NULL" : "NOT NULL"}`).join("\n")
    ...
    async selectPage(db, t, opt) {
      ...
      const orderSql = opt.orderBy ? ` ORDER BY ${qMssql(opt.orderBy)} ${opt.dir === "desc" ? "DESC" : "ASC"} OFFSET @off ROWS FETCH NEXT @lim ROWS ONLY` : ` ORDER BY (SELECT NULL) OFFSET @off ROWS FETCH NEXT @lim ROWS ONLY`;
      const pageSql = `SELECT * FROM ${fq}${orderSql}`;
      const r = await pool.request().input("off", mssql.Int, Math.max(Number(opt.offset) || 0, 0)).input("lim", mssql.Int, Math.min(Number(opt.limit) || 50, MAX_PAGE_ROWS)).query(pageSql);
...
async function mongoAdapter(cfg) {
  const mod = await loadDriver("mongodb");
  ...
  await withTimeout(client.connect(), CONNECT_TIMEOUT_MS + 3e3, "connect");
  return {
    kind: "mongodb",
    dialect: "mongo",
    async listDatabases() {
    ...
    async selectPage(db, t, opt) {
      ...
      const docsRaw = await coll.find(filter).skip(Math.max(Number(opt.offset) || 0, 0)).limit(Math.min(Number(opt.limit) || 50, MAX_PAGE_ROWS)).toArray();
      ...
      let docs;
      try {
        docs = JSON.parse(JSON.stringify(docsRaw, replacer));
      } catch (err) {
        throw new Error(`Failed to serialise documents: ${err.message}`);
      }
    ...
    async docSave(db, t, id, docJson) {
      ...
      try {
        ...
        throw new Error(`Failed to parse document JSON: ${err.message}`);
      }
      ...
      const rawId = id === null || typeof id === "string" || typeof id === "number" || typeof id === "boolean" || typeof id === "object" ? id : String(id);
      const coll = client.db(db).collection(t);
      const next = { ...body, _id: toId(rawId) };
    ...
    async docDelete(db, t, id) {
      ...
      const rawId = id === null || typeof id === "string" || typeof id === "number" || typeof id === "boolean" || typeof id === "object" ? id : String(id);
      const r = await client.db(db).collection(t).deleteOne({ _id: toId(rawId) });
...
async function redisAdapter(cfg) {
  const mod = await loadDriver("ioredis");
  ...
  await withTimeout(cli.ping(), CONNECT_TIMEOUT_MS + 2e3, "connect");
  async function scanKeys(pattern, cursorIn, wantIn) {
    const want = Math.min(Number(wantIn) || 200, 1e3);
  ...
  async function keyDetail(key) {
    ...
    let truncated = false;
    if (value.length > 64e3) {
      value = value.slice(0, 64e3) + "\n...[truncated]";
      truncated = true;
    }
    const size = type === "string" ? await cli.strlen(key) : type === "hash" ? await cli.hlen(key) : type === "list" ? await cli.llen(key) : type === "set" ? await cli.scard(key) : type === "zset" ? await cli.zcard(key) : type === "stream" ? await cli.xlen(key) : 0;
...
var entry = {
  activate(host) {
    const st = {
      conns: [],
      // [{id,name,type,host,port,user,password,database,file,uri,redisDb}]
      runtime: /* @__PURE__ */ new Map(),
      // connId -> {connId, ownerId, hostId, label, adapter}
      nextConnId: 1,
      depsOk: false,
    ...
    async function saveConfig() {
      const conns = sec ? st.conns.map((c) => ({ ...c, password: void 0 })) : st.conns;
    ...
    function installLocked(dir) {
      try {
        const parsed = JSON.parse(readFileSync(join(dir, INSTALL_LOCK), "utf8"));
    ...
    function installDeps(auto = false) {
      ...
      const child = npmCli ? spawn(process.execPath, [npmCli, ...args], { stdio: ["ignore", "ignore", "pipe"] }) : spawn(process.platform === "win32" ? "npm.cmd" : "npm", args, {
        stdio: ["ignore", "ignore", "pipe"]
      });
      st.installer = child;
      const timer = setTimeout(() => {
        host.log(`install timed out after ${INSTALL_TIMEOUT_MS / 6e4}min, killing npm`);
        try {
          child.kill("SIGTERM");
        } catch {
        }
      ...
      child.stderr?.on("data", (d) => {
        errTail = (errTail + d.toString()).slice(-1e3);
      });
      let done = false;
      ...
      async function finish(ok, why) {
        ...
        host.notify(
          ok ? "success" : "error",
          ok ? "🗄️ Database plugin: driver install finished" : `🗄️ Database plugin: driver install failed (${why}${lastErr ? `: ${lastErr}` : ""}) - run this manually in the plugin directory: npm install ${DEPS.join(" ")}`
        );
        broadcastAll();
      }
    }
    ...
    async function openAdapter(cfg) {
      const engine = isDbEngine(cfg.type) ? cfg.type : void 0;
      const factory = engine === void 0 ? void 0 : ADAPTER_FACTORIES[engine];
      if (engine === void 0 || !factory) throw new Error(`Unknown database type: ${String(cfg.type)}`);
      await refreshDeps();
        ...
        switch (action) {
          ...
          case "conns_save": {
            ...
            if (c.id) {
              ...
              storeConnSecret(c.id, c.password === null ? null : c.password || void 0);
              st.conns[i] = {
                ...old,
                name: c.name ?? old.name,
                type: old.type,
                // the engine cannot be changed (driver semantics differ too much)
                ...
                password: c.password === null ? void 0 : c.password || old.password,
                database: c.database ?? old.database,
                file: c.file ?? old.file,
                uri: c.uri === null ? void 0 : c.uri || old.uri,
              ...
              storeConnSecret(id, c.password || void 0);
              st.conns.push({
                id,
                ...
                user: c.user ?? "",
                password: c.password ? String(c.password) : void 0,
                database: c.database ?? "",
                file: c.file ?? "",
                uri: c.uri ? String(c.uri).trim() : void 0,
          ...
          case "connect": {
            ...
            for (const r2 of st.runtime.values()) {
              if (r2.hostId === cfg.id) {
                return void respond(action, reqId, clientId, {
                  connId: r2.connId,
                  label: r2.label,
                  kind: r2.adapter.kind,
                  dialect: r2.adapter.dialect
                });
              }
            }
