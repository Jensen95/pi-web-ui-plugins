/* Generated by scripts/build-plugins.mjs from plugins/db-client/src/client.ts - do not edit. */

// plugins/db-client/src/client.ts
...
var reqSeq = 0;
function isConnectionKind(value) {
...
<div class="dbx">
    ...
    function syncTreeToggle() {
      ...
      setMarkup(
        treeToggle,
        `<span>${open ? "▾" : "▸"}</span><span class="tn">${work?.curTable ? esc(`${work.curDb}.${work.curTable}`) : "Select a table..."}</span>`
      );
    }
    ...
    function request(payload) {
      ...
      return new Promise((resolve) => {
        pending.set(reqId, (response) => resolve(response));
    ...
    function renderConns() {
      ...
      for (const c of state.conns) {
        ...
        setMarkup(
          row,
          `<span class="dot ${isActive ? "on" : ""}"></span><span class="info"><span class="nm">${esc(c.name)}</span><span class="addr">${esc(connAddr2(c))}</span></span><span class="ops"><button data-op="edit" title="Edit">✎</button><button data-op="del" title="Delete">🗑</button></span>`
        );
    ...
    function renderTables() {
      ...
      for (const t of work.tables) {
        ...
        setMarkup(
          row,
          `<span>${t.kind === "view" ? "👁" : t.kind === "collection" ? "📄" : "▤"}</span><span class="tn" title="${esc(t.name)}">${esc(t.name)}</span>` + (t.approxRows && t.approxRows > 0 ? `<span class="cnt">${fmtCount2(t.approxRows)}</span>` : "") + (t.kind === "collection" ? '<span class="badge">coll</span>' : t.kind === "view" ? '<span class="badge">view</span>' : "")
        );
        row.addEventListener("click", () => selectTable(t.name, t.kind));
        tablesEl.appendChild(row);
      }
    }
    function fmtCount2(n) {
    ...
    async function loadData() {
      ...
      statusLine.textContent = `${work.curTable} · total ${fmtCount2(p.total)}  rows` + (r.grid.rows.length ? ` · this page ${r.grid.rows.length}  rows` : "");
      $(".pg-first").disabled = $(".pg-prev").disabled = p.no <= 0;
      $(".pg-next").disabled = $(".pg-last").disabled = p.no >= pages - 1;
    }
    function renderGrid(host, columns, rows, kind, grid = { columns: [], rows: [] }) {
      host.textContent = "";
      ...
      setMarkup(
        ...
        "<thead><tr>" + columns.map(
          (c) => `<th class="${canSort ? "sortable" : ""}" data-col="${esc(c)}">${esc(c)}${work?.orderBy === c ? work.dir === "desc" ? " ↓" : " ↑" : ""}</th>`
        ).join("") + (canDel ? '<th class="ops-th"></th>' : "") + "</tr></thead>"
      );
      if (canSort) {
      ...
      rows.forEach((row) => {
        ...
        setMarkup(
          ...
          row.map((v, ci) => {
            ...
            return canEditCell && ci !== pkIdx ? base.replace(
              "<td",
    ...
    async function openInsertModal() {
      ...
      setMarkup(
        rowBody,
        `<div class="flds">${cols.map(
          (c) => `<label>${esc(c.name)}<small>${esc(c.type)}${c.nullable ? "" : " · NOT NULL"}</small></label><input data-col="${esc(c.name)}" spellcheck="false" placeholder="${c.nullable ? "Blank = default" : "Required"}" />`
        ).join("")}</div>`
      );
    ...
    async function loadSchema() {
      ...
      if (d.columns.length) {
        ...
        setMarkup(
          t,
          "<thead><tr><th>#</th><th>Column name</th><th>Type</th><th>Null</th><th>Key</th><th>Default</th><th>Notes</th></tr></thead><tbody>" + d.columns.map(
            (c, i) => `<tr><td>${i + 1}</td><td>${esc(c.name)}</td><td>${esc(c.type)}</td><td>${c.nullable ? "YES" : "NO"}</td><td>${c.key ? `<span class="keytag">${esc(c.key)}</span>` : ""}</td><td>${esc(c.def ?? "")}</td><td>${esc(c.comment || "")}</td></tr>`
          ).join("") + "</tbody>"
        );
      ...
      if (d.indexes?.length) {
        ...
        setMarkup(
          t,
          "<thead><tr><th>Index name</th><th>Unique</th><th>Columns / Definition</th></tr></thead><tbody>" + d.indexes.map((i) => `<tr><td>${esc(i.name)}</td><td>${i.unique ? "✓" : ""}</td><td>${esc(i.columns)}</td></tr>`).join("") + "</tbody>"
        );
    ...
    async function runQuery() {
      ...
      qStatus.textContent = `✓ ${r.grid.elapsedMs}ms` + (r.grid.total ? ` · ${r.grid.total}  rows` : "") + (r.grid.affected ? ` · affected ${r.grid.affected}  rows` : "");
    ...
    async function redisKeyDetail(key) {
      ...
      const d = r.detail;
      kdInfo.textContent = `Type ${d.type} · Size ${fmtCount2(d.size)} · TTL ${d.ttl < 0 ? "∞" : `${d.ttl}s`}`;
    ...
    async function refreshRedisMeta() {
      ...
      if (r.ok) $(".redis-meta").textContent = `${fmtCount2(r.meta.dbsize)} keys · mem ${r.meta.usedMemory}`;
    }
    ...
    tabsEl.addEventListener("click", (ev) => {
      ...
      if (t && !t.classList.contains("hidden")) setTab(t.dataset.tab ?? "data");
    });
    ...
    function openModal(conn) {
      ...
      qf("uri").value = "";
      qf("uri").placeholder = conn?.hasUri ? "Saved (leave blank to keep it unchanged)" : "mongodb://user:pass@host:27017";
