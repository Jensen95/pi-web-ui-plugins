 ...
 * image-toolkit  ——
 *
 * ESM  { mount(container, ctx) → cleanup? } DOM
 *  import npm  Canvas
 *
 *
 *
 ...
 *
 * item.state  JSON  → /
 *  state renderToCanvas
 *
 */
import { detectLang, makeT } from "../client/i18n.mjs";
import {
	clamp,
	...
	makeZip,
	stem,
} from "../client/util.mjs";
import { drawHistogram, imageStats, parseExif, sniffType } from "../client/probe.mjs";
import * as ws from "../client/workspace.mjs";
import { createCropper } from "../client/crop.mjs";
import { shapeCommands, toPathD } from "../client/shapes.mjs";
import {
	DEFAULT_CROP_RADIUS,
	alphaFixForShape,
	RATIO_KEYS,
...
export default {
	mount(container, ctx) {
		...
		async function addFiles(files, opts = {}) {
			const list = [...files].filter((f) => f.type.startsWith("image/") || ws.IMAGE_EXT.test(f.name || ""));
		...
		function buildControl(c, item) {
			...
			// range/num/select/bool/color/text path  get/set
			// buttons/note/info/file —— undefined path
			// getter syncPanelValues
		...
		function buildShell() {
			...
			//
			// draggable=false + CSS -webkit-user-drag:noneChrome  <canvas>
			//  +
