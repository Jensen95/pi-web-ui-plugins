 ...
 *
 *
 *   - PNGIHDR//+ tRNS  + acTL  APNG
 *   - JPEG marker  SOFn /
 *   - GIF +  GCE
 *   - WEBPVP8X/VP8/VP8L  alpha / animation
 *   - BMPBITMAPINFOHEADER/ height = top-down
 *   - AVIFftyp  + ispe
 *   - SVG width/height  viewBox null
 * format: "unknown"width/height  nullmime  octet-stream
 */

import { Buffer } from "node:buffer";
import { mimeForFormat, sniffFormat } from "./codec.ts";
...
/**  format:"unknown"  width/height  null */
...
function tiffReader(buf, base) {
	...
	/**  IFD Map<tag, {type, count, values, text}> */
...
/** EXIF  "2024:05:01 12:00:00" → "2024-05-01 12:00:00" */
...
/**
 *  EXIFJPEG  APP1Exif\0\0 + TIFF PNG  eXIf
 *  IFD0 / ExifIFD / GPS IFD EXIF  null
 * exposureTime / fNumber / focalLength "1/250""f/2.8""35mm"
 */
