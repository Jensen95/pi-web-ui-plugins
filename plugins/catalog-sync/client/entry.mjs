...
var SYNC_TITLE = "Sync catalog";
var UPDATE_TITLE = "Install/update selected";
var COMMAND_TITLE = UPDATE_TITLE;
...
${body}
`;
var SYNC_SCRIPT = CATALOG_SCRIPT(`
${CATALOG_FILE_SCRIPT}
writeCatalog(entries);
`);
var UPDATE_SCRIPT = (requestedIds) => CATALOG_SCRIPT(`
...
function syncCatalogCommand() {
  return nodeCommand(SYNC_SCRIPT);
}
function updateSelectedCommand(requestedIds) {
...
function parseCatalog(value) {
  ...
  return value.map((item) => {
    ...
    return {
      ...
      homepage: typeof entry.homepage === "string" ? entry.homepage : void 0
    };
  });
}
...
var clientEntry = {
  mount(container) {
    ...
    const sendCommand = (title, command, success) => {
    ...
    void fetchCatalog().then((loadedEntries) => {
      if (disposed) return;
